...
# print(type(data_province))
print(data_province[0].keys())
chinaDayList = data['chinaDayList'] # 取出chinaDayList
# tb = pd.DataFrame(data_province).head() # 直接生成数据效果并不理想
# print(tb)
#
# info = pd.DataFrame(data_province)[['id','lastUpdateTime','name']].head()
# print(info)
...
def history_privince():
    ...
    for province_id in province_dict:  # 遍历各省编号
        ...
        try:
            ...
            print('-' * 20, province_dict[province_id], '成功',province_data.shape, alltime_province.shape,',累计耗时:', round(time.time() - start), '-' * 20)
...
def history_world():
    ...
    for country_id in country_dict:  # 遍历每个国家的编号
        ...
        try:
            ...
            print('-' * 20, country_dict[country_id], '成功', country_data.shape, alltime_world.shape,',累计耗时:', round(time.time() - start), '-' * 20)
